...
const calculateScore = (hand) => {
  ...
  let value = 0;
  let aces = 0;
  const validHand = hand.filter(c => c && c.val);
...
const PlayingCard = ({ card, hidden = false, index, isSmall = false }) => {
  if (!card) return null; // BLINDAJE CONTRA CRASHES
...
export default function BlackjackPage() {
  ...
  const cancelLobbyGame = async (gameId) => {
      ...
      try {
          ...
          setPvpLobbyGames(prev => prev.filter(g => g.id !== gameId));
      } catch (err) {
          console.error(err);
      }
      setIsProcessing(false);
  ...
  const hostProposeBet = async () => {
      ...
      try {
          ...
          const dp = JSON.parse(JSON.stringify(activePvPGame.datos_partida));
          dp.currency = currency;
          dp.betAmount = currency === 'green' ? betAmount : totalPetsValue;
  ...
  const leaveTable = async () => {
      ...
      if (activePvPGame.estado === 'playing') {
          return alert("You cannot leave an ongoing live match!");
      }
  ...
  const handleClaimLoot = () => {
      ...
      setTimeout(() => {
          ...
          if(gameMode === 'lobby') setGameMode('lobby'); 
      }, 800);
  };
                                ...
                                <div key={game.id} className="bg-[#0b0e14] border border-[#252839] p-3 md:p-4 rounded-xl flex items-center justify-between hover:border-[#6C63FF]/50 transition-colors group">
                                    <div className="flex items-center gap-3">
                                        <div className="relative">
                                            <img src={game.datos_partida?.p1?.avatar || '/default-avatar.png'} className="w-10 h-10 rounded-full border-2 border-[#252839]" />
                                        ...
                                        <div>
                                            <p className="text-xs md:text-sm font-bold text-white line-clamp-1">{game.datos_partida?.p1?.name || 'Player'}'s Table</p>
                                    ...
                                    <button 
                                        onClick={() => {
                                            if (game.creador_id === currentUser?.id) {
                                                cancelLobbyGame(game.id);
                                            } else {
                                                joinPvPGame(game);
                                            }
                                        }}
                                        disabled={(game.creador_id !== currentUser?.id && game.estado !== 'waiting') || isProcessing} 
                                        className="px-4 py-2 bg-[#1c1f2e] hover:bg-[#252839] border border-[#3b405a] text-white font-black uppercase text-[10px] md:text-xs rounded-lg disabled:opacity-50 transition-all group-hover:border-[#6C63FF]/50"
                                    >
                                        {game.creador_id === currentUser?.id ? 'Cancel Match' : game.estado === 'waiting' ? 'Sit Down' : 'Full'}
                                    </button>
                                </div>
                            ))
                        )}
                  ...
                  {(activePvPGame.estado === 'waiting' || activePvPGame.estado === 'negotiating' || activePvPGame.estado === 'host_ready') && (
                      <div className="flex flex-col md:flex-row items-center justify-center gap-8 md:gap-12 flex-1 w-full max-w-5xl mx-auto">
                          ...
                          <div className={`flex-1 flex flex-col items-center p-6 md:p-10 rounded-3xl border-2 transition-all w-full ${isHost ? 'bg-[#1c1f2e] border-[#6C63FF] shadow-[0_0_30px_rgba(108,99,255,0.2)]' : 'bg-[#0b0e14] border-[#252839]'}`}>
                              <img src={activePvPGame.datos_partida?.p1?.avatar || '/default-avatar.png'} className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-[#252839] mb-4 object-cover shadow-xl"/>
                              <h3 className="text-xl md:text-2xl font-black text-white uppercase mb-1 text-center w-full truncate">{activePvPGame.datos_partida?.p1?.name || 'Host'}</h3>
                          ...
                          <div className={`flex-1 flex flex-col items-center p-6 md:p-10 rounded-3xl border-2 transition-all w-full ${!isHost && activePvPGame.estado !== 'waiting' ? 'bg-[#1c1f2e] border-[#ef4444] shadow-[0_0_30px_rgba(239,68,68,0.2)]' : 'bg-[#0b0e14] border-[#252839]'}`}>
                              {activePvPGame.estado === 'waiting' ? (
                                  ...
                                  <img src={activePvPGame.datos_partida?.p2?.avatar || '/default-avatar.png'} className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-[#252839] mb-4 object-cover shadow-xl animate-fade-in"/>
                              )}
                              
                              <h3 className="text-xl md:text-2xl font-black text-[#8f9ac6] uppercase mb-1 text-center w-full truncate">
                                  {activePvPGame.estado === 'waiting' ? 'Empty Seat' : activePvPGame.datos_partida?.p2?.name || 'Loading...'}
                              </h3>
                      ...
                      <div className="flex-1 flex flex-col justify-between w-full h-full animate-fade-in">
                          ...
                          <div className="w-full flex flex-col items-center">
                              <div className="flex items-center gap-3 mb-4">
                                  <img src={activePvPGame.datos_partida?.p2?.avatar || '/default-avatar.png'} className="w-10 h-10 rounded-full border-2 border-[#252839]"/>
                          ...
                          <div className="w-full flex flex-col items-center">
                              ...
                              <div className="flex items-center gap-3">
                                  <img src={activePvPGame.datos_partida?.p1?.avatar || '/default-avatar.png'} className="w-10 h-10 rounded-full border-2 border-[#252839]"/>
                                  <span className="px-4 py-1.5 bg-[#1c1f2e] border border-[#252839] rounded-full text-xs font-black uppercase tracking-widest text-white shadow-md">
