...
export default function CasesPage() {
  ...
  // Estados de Animación de Ruleta
  ...
  useEffect(() => {
    const initData = async () => {
        ...
        // 2. Obtener cajas dinámicas desde TU tabla `cases`
        const { data: cajas, error } = await supabase
            .from('cases')
            .select('*')
            .order('price', { ascending: true }); // Ordenadas de la más barata a la más cara
  ...
  const openCase = async () => {
    ...
    if (error || winner?.error) {
        setView('inspect');
        return alert("Error: " + (winner?.error || "Fallo en la conexión. Revisa tu saldo verde."));
    }

    setWinningItem(winner);

    // Generar la cinta de la ruleta (llenamos 60 espacios con items de la caja seleccionada)
    const track = [];
    for (let i = 0; i < 60; i++) {
    ...
    // Iniciar el giro con un ligero retraso
    setTimeout(() => {
  ...
  // Helper para extraer datos de los items (soporta los nombres en español de tu JSON)
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#0b0e14] text-white p-4 md:p-8 relative overflow-hidden animate-fade-in">
      <div className="max-w-[1200px] mx-auto relative z-10">
        ...
        {/* =========================================
            VISTA 1: TIENDA (DINÁMICA DESDE DB)
        ========================================= */}
        {view === 'store' && (
          <div>
            ...
            {loadingCases ? (
                <div className="py-20 flex flex-col items-center justify-center">
                    ...
                    <p className="text-[#555b82] font-black uppercase tracking-widest text-xs">Loading cases from database...</p>
                </div>
            ) : casesDB.length === 0 ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
                  {casesDB.map(caja => (
                    ...
                    >
                      ...
                      {/* Usando caja.image_url como en tu base de datos */}
        ...
        {view === 'inspect' && selectedCase && (
          <div className="animate-fade-in">
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              {/* Panel Izq: Abrir Caja (Diseño consistente) */}
              <div className="w-full lg:w-1/3 bg-[#1c1f2e] border border-[#252839] rounded-2xl p-8 flex flex-col items-center relative overflow-hidden shadow-xl h-max">
                <div className="absolute inset-0 opacity-10" style={{ background: `radial-gradient(circle at center, ${selectedCase.color} 0%, transparent 70%)` }}></div>
              ...
              <div className="w-full lg:w-2/3 bg-[#1c1f2e] border border-[#252839] rounded-2xl p-6 shadow-xl">
                <div className="flex items-center justify-between border-b border-[#252839] pb-4 mb-6">
                    ...
                    <span className="text-[#555b82] font-black text-xs uppercase">{selectedCase.items.length} Items</span>
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                  {selectedCase.items.map((rawItem, i) => {
                    ...
                    <div key={i} className="bg-[#141323] border border-[#2F3347] rounded-xl overflow-hidden flex flex-col relative group transition-colors hover:border-[#4f567a] h-44">
                      ...
                      <div className="absolute top-2 right-2 bg-[#1c1f2e] border border-[#252839] px-1.5 py-0.5 rounded text-[9px] font-black text-[#8f9ac6] z-20">
                        {item.chance}%
                      </div>
        ...
        {view === 'opening' && (
          <div className="w-full flex flex-col items-center justify-center min-h-[50vh] animate-fade-in relative z-20">
            ...
            <div className={`w-full relative h-[200px] bg-[#141323] border-y-2 border-[#252839] overflow-hidden shadow-inner flex items-center transition-opacity duration-300 rounded-lg ${spinning ? 'opacity-100' : 'opacity-0'}`}>
              ...
              {/* Sombras laterales para ocultar los bordes */}
        ...
        {view === 'result' && winningItem && (() => {
          ...
          <div className="w-full flex flex-col items-center justify-center min-h-[50vh] animate-scale-in relative z-20">
            ...
            <div className="relative w-64 h-[300px] bg-[#141323] border-2 rounded-2xl p-6 flex flex-col items-center justify-center shadow-2xl z-10 overflow-hidden" style={{borderColor: wItem.color, boxShadow: `0 0 40px ${wItem.color}30`}}>
              ...
              <div className="absolute top-3 right-3 bg-[#1c1f2e] border border-[#252839] px-2 py-1 rounded text-[10px] font-black text-[#8f9ac6] z-20">
                 {wItem.chance}%
              </div>
