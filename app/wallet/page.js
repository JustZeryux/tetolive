...
export default function WalletPage() {
  ...
  // --- LÓGICA DE VENTA (COMISIÓN PROGRESIVA) ---
  const calcularVenta = (valorOriginal) => {
    let feePercentage = 0.10; // 10% por defecto (pets baratas)
    
    // Entre más cara la pet, menos te cobran de comisión wey
    if (valorOriginal > 1000000) {
      feePercentage = 0.02; // 2% para pets arriba de 1M
    } else if (valorOriginal > 500000) {
      feePercentage = 0.04; // 4% para pets arriba de 500k
    } else if (valorOriginal > 100000) {
      feePercentage = 0.06; // 6% para pets arriba de 100k
    } else if (valorOriginal > 10000) {
      feePercentage = 0.08; // 8% para pets arriba de 10k
    }
  ...
  const venderPet = async (invId, itemId, petName, originalPrice) => {
    if (procesando) return;
    
    const { sellPrice, feePercentage } = calcularVenta(originalPrice);
    
    if (!confirm(`¿Vender ${petName} instantáneamente?\nEl sistema cobrará ${feePercentage}% de comisión.\nRecibirás: ${sellPrice.toLocaleString()} 🟢`)) return;
    
    setProcesando(true);
    
    try {
      const nuevoSaldo = saldoVerde + sellPrice;
      const { error: err1 } = await supabase.from('profiles').update({ saldo_verde: nuevoSaldo }).eq('id', currentUser.id);
      if (err1) throw err1;

      const { error: err2 } = await supabase.from('inventory').delete().eq('id', invId);
      if (err2) throw err2;

      const { error: err3 } = await supabase.from('marketplace').insert({
      ...
      alert(`¡Venta Exitosa!\nRecibiste: ${sellPrice.toLocaleString()} 🟢\nTu item ahora está en el mercado global.`);
      await fetchData(); 
    } catch (error) {
      console.error("Error al vender:", error);
      alert("Error al intentar vender la mascota.");
    }
  ...
  const comprarDelMarket = async (marketId, petName, precio, itemId) => {
    ...
    try {
      ...
      const { error: err2 } = await supabase.from('inventory').insert({
        user_id: currentUser.id,
        item_id: itemId
      });
      if (err2) throw err2;

      const { error: err3 } = await supabase.from('marketplace').delete().eq('id', marketId);
      if (err3) throw err3;
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-[#0b0e14] text-white p-4 md:p-8 animate-fade-in relative overflow-hidden font-sans">
      ...
      <div className="max-w-[1200px] mx-auto relative z-10">
        ...
        <div className="bg-[#14151f]/80 backdrop-blur-xl border border-[#252839] rounded-[2rem] p-6 md:p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] min-h-[500px] relative z-10">
            ...
            {/* Teto Decorativa en el fondo del panel */}
            <img 
              src="/teto.png" 
              alt="Teto Decoración" 
              className="absolute -right-10 -bottom-10 w-64 opacity-[0.03] pointer-events-none z-0"
            ...
            {cargando ? (
                ...
                /* GRID DE ITEMS CON DISEÑO CABRÓN */
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-5 max-h-[650px] overflow-y-auto custom-scrollbar pr-2 pb-6 relative z-10">
                    ...
                    {tab === 'inventory' && filteredInventory.map((pet, i) => {
                      ...
                      return (
                        <div key={pet.inventarioId} className="relative bg-[#0b0e14] border-2 border-[#252839] rounded-2xl p-4 flex flex-col items-center justify-between transition-all duration-300 hover:border-[#6C63FF]/50 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(0,0,0,0.6)] group overflow-hidden h-56 animate-fade-in-up" style={{animationDelay: `${i * 30}ms`}}>
                            
                            {/* Gradiente dinámico de fondo */}
                            <div className="absolute inset-0 opacity-10 group-hover:opacity-30 transition-opacity duration-500" style={{ background: `radial-gradient(circle at top, ${pet.color} 0%, transparent 80%)` }}></div>
                            
                            {/* Resplandor al hacer hover */}
                            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-xl pointer-events-none" style={{ boxShadow: `inset 0 0 40px ${pet.color}20` }}></div>

                            <img src={pet.img} className="w-20 h-20 object-contain mt-2 mb-3 z-10 drop-shadow-[0_10px_15px_rgba(0,0,0,0.8)] group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-500" alt="pet"/>
                            
                            <div className="w-full text-center z-10 mb-3 bg-[#14151f]/60 rounded-xl py-2 px-1 backdrop-blur-sm border border-[#252839]/50 group-hover:border-[#252839] transition-colors">
                            ...
                            <button 
                                onClick={() => venderPet(pet.inventarioId, pet.itemId, pet.nombre, pet.valor)}
                                disabled={procesando}
                                className="w-full bg-[#1c1f2e] hover:bg-red-500 text-[#ef4444] hover:text-white border border-[#252839] hover:border-red-500 text-[11px] font-black uppercase tracking-widest py-2.5 rounded-xl transition-all duration-300 z-10 disabled:opacity-50 hover:shadow-[0_0_15px_rgba(239,68,68,0.5)] flex items-center justify-center gap-1"
                            >
                                SELL: <GreenCoin cls="w-3 h-3"/> {formatValue(sellPrice)}
                            </button>
                        </div>
                      );
                    })}
                    ...
                    {tab === 'market' && filteredMarket.map((m, i) => (
                        <div key={m.marketId} className="relative bg-[#0b0e14] border-2 border-[#252839] hover:border-[#22c55e]/50 rounded-2xl p-4 flex flex-col items-center justify-between transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(34,197,94,0.15)] group overflow-hidden h-56 animate-fade-in-up" style={{animationDelay: `${i * 30}ms`}}>
                            
                            <div className="absolute inset-0 opacity-10 group-hover:opacity-30 transition-opacity duration-500" style={{ background: `radial-gradient(circle at top, ${m.color} 0%, transparent 80%)` }}></div>
                            ...
                            <img src={m.img} className="w-20 h-20 object-contain mt-3 mb-3 z-10 drop-shadow-[0_10px_15px_rgba(0,0,0,0.8)] group-hover:scale-110 transition-transform duration-500" alt="pet" style={{filter: `drop-shadow(0 0 15px ${m.color}40)`}}/>
                            
                            <div className="w-full text-center z-10 mb-3 bg-[#14151f]/60 rounded-xl py-2 px-1 backdrop-blur-sm border border-[#252839]/50">
                            ...
                            <button 
                                ...
                                disabled={procesando}
                                className="w-full bg-gradient-to-r from-[#22c55e] to-[#16a34a] hover:from-[#16a34a] hover:to-[#15803d] text-[#0b0e14] text-[11px] font-black uppercase tracking-widest py-2.5 rounded-xl transition-all duration-300 z-10 shadow-[0_0_15px_rgba(34,197,94,0.3)] disabled:opacity-50"
                            >
                                BUY PET
                            </button>
                        </div>
